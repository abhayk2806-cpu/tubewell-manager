...
const MonthsPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    const months = [...new Set((e || []).map(x => x.month))];
  ...
  // Build month summary for a farmer or all farmers
  const getMonthData = (month: string, farmerId?: string) => {
    const monthEntries = entries.filter(e => e.month === month && (!farmerId || e.farmer_id === farmerId));
    ...
    // For payments, use all-time payments and calculate how much applies to this month
    // Simplified: show total payments made in this month
    const monthPayments = payments.filter(p => {
      const pMonth = format(new Date(p.date), 'MMMM yyyy');
      return pMonth === month && (!farmerId || p.farmer_id === farmerId);
    });
  ...
  const overallData = selectedMonth ? getMonthData(selectedMonth, filterFarmer || undefined) : null;
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white rounded-2xl p-4 border shadow-sm" style={{ borderColor: '#e5e2dc' }}>
              ...
              <div className="text-xl font-bold text-blue-600">{overallData.totalHours}h {overallData.totalMins}m</div>
            </div>
            <div className="bg-white rounded-2xl p-4 border shadow-sm" style={{ borderColor: '#e5e2dc' }}>
              <div className="text-xs text-gray-500 mb-1">Total Amount</div>
              <div className="text-xl font-bold text-gray-900">₹{overallData.totalAmount.toLocaleString('en-IN')}</div>
            </div>
            <div className="bg-white rounded-2xl p-4 border shadow-sm" style={{ borderColor: '#e5e2dc' }}>
              <div className="text-xs text-gray-500 mb-1">Paid (this month)</div>
              <div className="text-xl font-bold text-green-600">₹{overallData.totalPaid.toLocaleString('en-IN')}</div>
            </div>
            <div className="bg-white rounded-2xl p-4 border shadow-sm" style={{ borderColor: '#e5e2dc' }}>
              <div className="text-xs text-gray-500 mb-1">Remaining</div>
              <div className="text-xl font-bold text-red-500">₹{overallData.remaining.toLocaleString('en-IN')}</div>
            </div>
          </div>
          ...
          {overallData.totalAmount > 0 && (
            <div className="bg-white rounded-2xl p-4 border shadow-sm" style={{ borderColor: '#e5e2dc' }}>
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="font-semibold">{Math.round((overallData.totalPaid / overallData.totalAmount) * 100)}%</span>
              </div>
              <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-green-500 rounded-full transition-all"
                  style={{ width: `${Math.min(100, (overallData.totalPaid / overallData.totalAmount) * 100)}%` }} />
              </div>
            </div>
          )}
                    ...
                    <div key={f.id} className="px-4 py-3">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="text-xs text-gray-400">{f.totalHours}h {f.totalMins}m • {f.entries.length} entries</div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm font-bold text-gray-900">₹{f.totalAmount.toLocaleString('en-IN')}</div>
                          {f.remaining > 0 && (
                            <div className="text-xs text-red-500">Baki: ₹{f.remaining.toLocaleString('en-IN')}</div>
                          )}
                        </div>
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-2xl border shadow-sm overflow-hidden" style={{ borderColor: '#e5e2dc' }}>
            <div className="px-4 py-3 border-b font-semibold text-gray-800" style={{ borderColor: '#e5e2dc' }}>
              Entries ({overallData.entries.length})
            </div>
                  ...
                  <div key={e.id} className="px-4 py-3">
                    <div className="flex justify-between">
                      <div>
                        ...
                        <div className="text-xs text-gray-400">{format(new Date(e.date), 'dd MMM, hh:mm a')}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-bold text-gray-900">₹{Number(e.amount).toLocaleString('en-IN')}</div>
